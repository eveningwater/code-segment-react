...
渲染一个带有动画效果的可以配置图片选项或者文字选项的选项卡组件。

- 使用`useState`管理选项，图片选项或者文字选项是一个数组。
- 添加一个随机数作为选项卡的 key。
- 在`useEffect`函数中根据选项卡的类名获取该元素，并计算元素的 left 偏移量，调用`window.scrollTo`方法修改 left 偏移量，同时为线元素添加过渡动画的时间。
- 根据不同的条件渲染元素的类名。
