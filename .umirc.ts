import { defineConfig } from 'dumi';
import about from './config/about';
import menus from './config/menu';

export default defineConfig({
  title: 'react-code-segment',
  mode: 'site',
  base: process.env.NODE_ENV === 'production' ? '/code-segment-react/old/' : '/',
  outputPath: 'build',
  publicPath:
    process.env.NODE_ENV === 'production' ? '/code-segment-react/old/' : '/',
  logo: 'data:image/svg+xml;base64,',
  styles: [`img {margin: auto; display:block;}`],
  links: [
    {
      rel: 'stylesheet',
      type: 'text/css',
      href:
        process.env.NODE_ENV === 'production'
          ? '/code-segment-react/old/common.css'
          : '/common.css',
    },
  ],
  scripts: [
    {
      src: 'https://cdn.bootcdn.net/ajax/libs/vConsole/3.14.7/vconsole.min.js',
    },
    {
      src:
        process.env.NODE_ENV === 'production'
          ? '/code-segment-react/old/debug.js'
          : '/debug.js',
    },
  ],
  menus: menus,
  navs: {
    'en-US': [
      {
        title: 'getting-started',
        path: '/getting-started',
      },
      {
        title: 'guide',
        path: '/guide',
      },
      {
        title: 'hooks',
        path: '/hooks',
      },
      {
        title: 'model',
        path: '/model',
      },
      {
        title: 'antd',
        path: '/antd',
      },
      {
        title: 'github',
        path: 'https://github.com/eveningwater/code-segment-react.git',
      },
      {
        title: 'about me',
        path: 'https://www.eveningwater.com/my-web-projects/',
        children: about.en,
      },
    ],
    'zh-CN': [
      {
        title: '快速上手',
        path: '/zh-CN/getting-started',
      },
      {
        title: '指南',
        path: '/zh-CN/guide',
      },
      {
        title: '钩子函数',
        path: '/zh-CN/hooks',
      },
      {
        title: '状态管理',
        path: '/zh-CN/model',
      },
      {
        title: '蚂蚁设计',
        path: '/zh-CN/antd',
      },
      {
        title: '源码地址',
        path: 'https://github.com/eveningwater/code-segment-react.git',
      },
      {
        title: '关于我',
        path: 'https://www.eveningwater.com/my-web-projects/',
        children: about.zh,
      },
    ],
  },
});
